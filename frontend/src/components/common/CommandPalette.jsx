...
const getThemeItems = (t) => [
  { name: t.common.light, value: 'light', icon: Sun, keywords: ['bright', 'day'] },
  { name: t.common.dark, value: 'dark', icon: Moon, keywords: ['night'] },
  { name: t.common.system, value: 'system', icon: Monitor, keywords: ['auto', 'default'] },
]
...
export function CommandPalette() {
  ...
  const handleNavigate = useCallback((href) => {
    handleSelect(() => navigate(href))
  }, [handleSelect, navigate])

  const handleSearch = useCallback(() => {
    if (!query.trim()) return
    handleSelect(() => navigate(`/search?q=${encodeURIComponent(query)}&mode=search`))
  }, [handleSelect, navigate, query])

  const handleAsk = useCallback(() => {
    if (!query.trim()) return
    handleSelect(() => navigate(`/search?q=${encodeURIComponent(query)}&mode=ask`))
  }, [handleSelect, navigate, query])
