...
export function NotebookAssociations({
  ...
  const [selectedNotebookIds, setSelectedNotebookIds] = useState(currentNotebookIds)
            ...
            {notebooks
              ...
              .map((notebook) => {
                ...
                return (
                  <div
                    key={notebook.id}
                    className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${isSelected ? 'bg-accent border-accent-foreground/20' : 'hover:bg-accent/50'
                      }`}
                  >
                    <Checkbox
                      checked={isSelected}
